...
import { mockAlugueis, mockMotos } from "@/lib/mock-data";
import { formatCurrency, formatDate } from "@/lib/utils";

export default function AluguelPage() {
  const motosAluguel = mockMotos.filter(
    (m) => m.tipo === "aluguel" || m.tipo === "ambos"
  );
  const ativosCount = mockAlugueis.filter((a) => a.status === "ativo").length;
  ...
  const receitaMes = mockAlugueis.reduce((acc, a) => acc + a.valorTotal, 0);
    ...
    <div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-keu-black/5">
              {mockAlugueis.map((a) => (
                <tr key={a.id} className="hover:bg-keu-gray-light transition">
                  <td className="p-4">
      ...
      <Card>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <div className="relative">
                ...
                <Input placeholder="Buscar moto..." className="pl-9 w-48" />
              </div>
        ...
        <div className="p-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {motosAluguel.map((m) => (
            <Card key={m.id} className="overflow-hidden hover:shadow-xl transition-all">
...
function AluguelStatusBadge({ status }: { status: string }) {
  ...
  > = {
    ...
    concluido: { v: "success", l: "Concluído", i: <CheckCircle2 className="h-3 w-3" /> },
    atrasado: { v: "danger", l: "Atrasado", i: <AlertCircle className="h-3 w-3" /> },
