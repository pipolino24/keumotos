...
function motoToForm(m: MotoApi): MotoForm {
  return {
    marca: m.marca ?? "",
    modelo: m.modelo ?? "",
    versao: m.versao ?? "",
    ...
    potencia: m.potencia ?? "",
    km: m.km,
    combustivel: (m.combustivel as MotoForm["combustivel"]) ?? "",
    cambio: (m.cambio as MotoForm["cambio"]) ?? "",
    partida: (m.partida as MotoForm["partida"]) ?? "",
    ...
    valorMensal: n(m.valorMensal),
    caucao: n(m.caucao),
    tipo: m.tipo ?? "venda",
    status: m.status ?? "disponivel",
    setor: m.setor ?? "multimarcas",
    destaque: !!m.destaque,
...
export default function EditarMotoPage() {
  ...
  const { data, loading, error: loadError } = useApi<{ moto: MotoApi }>(
    id ? `/api/motos/${id}` : null
  );

  const [form, setForm] = useState<MotoForm | null>(null);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (data?.moto) {
      const t = setTimeout(() => setForm(motoToForm(data.moto)), 0);
  ...
  const valorMinimoN =
    typeof form?.valorMinimo === "number" ? form.valorMinimo : 0;
  const valorFipeN =
    typeof form?.valorFipe === "number" ? form.valorFipe : 0;
  const percentualFipe = valorFipeN
    ? ((valorAnunciadoN / valorFipeN - 1) * 100).toFixed(1)
    : "0";
  const margemMinima = valorMinimoN - valorFipeN;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/dashboard/estoque/${id}`);
    } catch (err) {
          ...
          <div className="flex items-center gap-3 text-red-700">
            ...
            <div>
              ...
              <p className="text-sm mt-1">
                {loadError ?? "Moto não encontrada ou removida."}
              </p>
            </div>
          </div>
        </Card>
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Link
        ...
        className="inline-flex items-center gap-2 text-sm font-medium text-keu-black/60 hover:text-keu-red transition mb-4"
      >
        <ArrowLeft className="h-4 w-4" /> Voltar pra moto
      </Link>

      <PageHeader
        title={
          <span>
            Editar — {form.marca} {form.modelo}
          </span>
        }
        description="Atualize dados, valores, fotos ou status. Origem e dados de compra/repasse são editados em Aquisições."
      >
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="p-6">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-keu-black/5">
              ...
              <div>
                ...
                <p className="text-sm text-keu-black/60">
                  Placa, RENAVAM, chassi e número do motor
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  value={form.chassi}
                  onChange={(e) =>
                    set("chassi", e.target.value.toUpperCase())
                  }
                />
              </div>
              <div>
          ...
          <Card className="p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="cilindrada" required>
                  Cilindrada (cc)
                </Label>
                <Input
                  id="cilindrada"
              ...
              <div>
                <Label htmlFor="combustivel" required>
                  Combustível
                </Label>
                <Select
                  id="combustivel"
          ...
          <Card className="p-6">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-keu-black/5">
              ...
              <div>
                ...
                <p className="text-sm text-keu-black/60">
                  FIPE, preço público, mínimo e comissão
                </p>
              </div>
            </div>
            ...
            {valorAnunciadoN > 0 && valorFipeN > 0 && (
              <div className="bg-gradient-to-br from-keu-gray-light to-white border border-keu-black/5 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3 text-xs font-semibold text-keu-black/60 uppercase tracking-wider">
                  <Info className="h-3 w-3" /> Análise vs FIPE
                </div>
          ...
          <Card className="p-6">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-keu-black/5">
              ...
              <div>
                ...
                <p className="text-sm text-keu-black/60">
                  Conteúdo do anúncio e anotações internas
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Textarea
                  id="observacoes"
                  rows={3}
                  placeholder="Não visíveis ao cliente"
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <p className="text-xs text-keu-black/60 mb-4">
              Imagens são comprimidas automaticamente antes de salvar (~250KB).
            </p>
            <ImageUpload
              value={form.fotos}
